...
exits unexpectedly.  Note that the worker script must call `exposeModelWorker`
(worker scripts generated by `@sdeverywhere/plugin-worker` always do); in a
browser environment, a worker script that instead runs to completion or closes
itself cannot be detected, and the returned promise would remain pending.
