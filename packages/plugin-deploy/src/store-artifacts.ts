...
export function storeArtifacts(
  ...
  try {
    ...
    // const productPaths = Object.entries(options.products).map(([name, product]) => {
    //   return {
    //     name,
    ...
    // }
    updateMetadata(context, options.baseUrl, currentBranchName, paths)
 ...
 */
function updateMetadata(context: BuildContext, baseUrl: string, currentBranchName: string, paths) {
  ...
  branchSpecs = branchSpecs.filter(spec => spec.name !== currentBranchName)

  // Add a new branch spec
  const newBranchSpec: BranchSpec = {
  ...
  branchSpecs.push(newBranchSpec)
  ...
  context.log('verbose', `Updated metadata for branch '${currentBranchName}'`)
...
function generateIndexHtml(context: BuildContext, branchSpecs: BranchSpec[]) {
  ...
  function getBranchLinksHtml(branch: BranchSpec): string {
    ...
    //   <a href="${branch.app}/">app</a>
    //   <span class="separator">|</span>
    //   <a href="${branch.checkReport}/">checks</a>
    // </div>
  }
