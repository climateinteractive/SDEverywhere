...
describe('generateEquation (Vensim -> JS)', () => {
  ...
  it('should work for constant definition (with two dimensions)', () => {
    ...
    `)
    expect(vars.size).toBe(3)
    expect(genJS(vars.get('_x[_a1,_dimc]'), 'init-constants')).toEqual([
      'for (let i = 0; i < 2; i++) {',
      '_x[0][i] = 2.0;',
      '}'
    ])
    expect(genJS(vars.get('_x[_a2,_dimc]'), 'init-constants')).toEqual([
      'for (let i = 0; i < 2; i++) {',
      '_x[1][i] = 1.0;',
      '}'
    ])
    expect(genJS(vars.get('_x[_a3,_dimc]'), 'init-constants')).toEqual([
      'for (let i = 0; i < 2; i++) {',
      '_x[2][i] = 1.0;',
      '}'
    ])
  ...
  it('should work for GET DATA AT TIME function', () => {
    const extData: ExtData = new Map([
      [
        '_x',
        new Map([
    ...
    expect(genJS(vars.get('_x'), 'decl', { extData })).toEqual(['const _x_data_ = [0.0, 0.0, 1.0, 2.0, 2.0, 5.0];'])
    expect(genJS(vars.get('_x'), 'init-lookups', { extData })).toEqual(['_x = fns.createLookup(3, _x_data_);'])
    expect(genJS(vars.get('_y'))).toEqual([`_y = fns.GET_DATA_AT_TIME(_x, 1.0);`])
  })
  ...
  it('should work for GET DIRECT CONSTANTS function (separate definitions)', () => {
    ...
    `)
    expect(vars.size).toBe(3)
    expect(genJS(vars.get('_x[_a1,_dimc]'), 'init-constants', { modelDir })).toEqual([
      'for (let i = 0; i < 2; i++) {',
      '_x[0][i] = 0.0;',
      '}'
    ])
    expect(genJS(vars.get('_x[_a2,_dimc]'), 'init-constants', { modelDir })).toEqual([
      '_x[1][0] = 12.0;',
      '_x[1][1] = 22.0;'
    ])
    expect(genJS(vars.get('_x[_a3,_dimc]'), 'init-constants', { modelDir })).toEqual([
      '_x[2][0] = 13.0;',
      '_x[2][1] = 23.0;'
    ])
  })
