...
describe('generateEquation (Vensim -> C)', () => {
  ...
  it.skip('should work for 2D equation with two dimensions that resolve to the same family used in expression position (separated/non-apply-to-all)', () => {
    ...
    expect(genC(vars.get('_x[_a1,_a2]'))).toEqual(['_x[0][1] = ((0 + 1) * 10.0) + (1 + 1);']) // BUG: RHS is generated as ((0 + 1) * 10.0) + (0 + 1)` (related to #179)
    expect(genC(vars.get('_x[_a2,_a1]'))).toEqual(['_x[1][0] = ((1 + 1) * 10.0) + (0 + 1);']) // BUG: RHS is generated as ((0 + 1) * 10.0) + (1 + 1)` (related to #179)
  ...
  it.skip('should work for 2D equation with two dimensions (including one subdimension) that resolve to the same family used in expression position (separated/non-apply-to-all)', () => {
    ...
    expect(genC(vars.get('_x[_a1,_a1]'))).toEqual(['_x[0][0] = 0.0;'])
    expect(genC(vars.get('_x[_a1,_a2]'))).toEqual(['_x[0][1] = ((0 + 1) * 10.0) + (1 + 1);']) // BUG: RHS is generated as ((0 + 1) * 10.0) + (0 + 1)` (related to #179)
    expect(genC(vars.get('_x[_a1,_a3]'))).toEqual(['_x[0][2] = ((0 + 1) * 10.0) + (2 + 1);']) // BUG: RHS is generated as ((0 + 1) * 10.0) + (0 + 1)` (related to #179)
    expect(genC(vars.get('_x[_a3,_a1]'))).toEqual(['_x[2][0] = ((2 + 1) * 10.0) + (0 + 1);']) // BUG: RHS is generated as ((2 + 1) * 10.0) + (2 + 1)` (related to #179)
    expect(genC(vars.get('_x[_a3,_a2]'))).toEqual(['_x[2][1] = ((2 + 1) * 10.0) + (1 + 1);']) // BUG: RHS is generated as ((2 + 1) * 10.0) + (2 + 1)` (related to #179)
  ...
  describe('when LHS is NON-apply-to-all (2D)', () => {
    ...
    it.skip('should work when RHS variable is NON-apply-to-all (1D) and is accessed with 2 different dimensions from LHS that map to the same family', () => {
      ...
      expect(genC(vars.get('_y[_a1,_a2]'))).toEqual(['_y[0][1] = _x[0] + _x[1];']) // BUG: RHS is generated as `_x[0] + _x[0]`
      expect(genC(vars.get('_y[_a2,_a1]'))).toEqual(['_y[1][0] = _x[1] + _x[0];']) // BUG: RHS is generated as `_x[1] + _x[1]`
