...
Of particular note, the [`ModelSpec`](./src/model-spec.js) type is the authoritative definition of the JSON "spec file" format that is passed to the `sde generate` command using the `--spec` argument, and every supported property is documented there.
