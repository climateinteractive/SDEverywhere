...
import type { GraphComparisonDatasetReport, GraphComparisonMetadataReport } from './diff-graphs'
import { diffGraphs } from './diff-graphs'
import type { ComparisonTestSummary } from '../report/comparison-report-types'

const scenarioKey = 'all_inputs_at_default' as ComparisonScenarioKey
...
function metadataReport(
  key: string,
  valueL: string | undefined,
  valueR: string | undefined
...
function datasetReport(datasetKey: DatasetKey, maxDiff: number): GraphComparisonDatasetReport {
  return {
    datasetKey,
    maxDiff
  }
...
describe('diffGraphs', () => {
  ...
  it('should return a report for two graphs with dataset differences', () => {
    ...
    const testSummaries: ComparisonTestSummary[] = [
      { d: 'Model__coal_amount', s: 'a_different_scenario' as ComparisonScenarioKey, md: 40 },
      { d: 'Model__coal_amount', s: scenarioKey, md: 50 },
      { d: 'Model__oil_amount', s: scenarioKey, md: 10 }
    ]

    const report = diffGraphs(graphL, graphR, scenarioKey, testSummaries)
    expect(report.inclusion).toBe('both')
    expect(report.metadataReports).toEqual([])
