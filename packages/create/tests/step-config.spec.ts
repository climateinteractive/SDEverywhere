...
async function runAndVerify(sampleModelContent: string, sampleModelExt: string) {
  ...
  writeFileSync(resolvePath(scratchDir, `sample.${sampleModelExt}`), sampleModelContent)
...
describe('step - read model variables and create config files', () => {
  it('should read model variables from a Vensim model and suggest input/output variables to include', async () => {
...
SAVEPER = TIME STEP ~~|
`
    await runAndVerify(sampleModelContent, 'mdl')
  })

  it('should read model variables from a Stella/XMILE model and suggest input/output variables to include', async () => {
    const sampleModelContent = `\
<xmile xmlns="http://docs.oasis-open.org/xmile/ns/XMILE/v1.0" version="1.0">
  ...
  <model>
    ...
    await runAndVerify(sampleModelContent, 'stmx')
  })
})
