...
The full set of supported properties is defined by the `ModelSpec` type in the [`@sdeverywhere/compile`](https://github.com/climateinteractive/SDEverywhere/blob/main/packages/compile/src/model-spec.d.ts) package, which is the authoritative reference.
