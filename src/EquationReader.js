...
module.exports = class EquationReader extends ModelReader {
  ...
  generateSmoothLevel(input, delay, init, levelNumber) {
    ...
    if (this.var.refId === '_r_d_breakthrough_effect_on_cost[_fcoal]') {
      debugger
    }
    if (isSeparatedVar(this.var)) {
      ...
      //   if (!sepDim && isIndex(subscript)) {
      //     for (let d of this.var.separationDims) {
      //       let indexNames = indexNamesForSubscript(d)
      //       if (indexNames.includes(subscript)) {
      //         sepDim = d
      //         index = subscript
      //         // sepDim = decanonicalize(d)
      //         // index = decanonicalize(subscript)
      //         break
      //       }
      //     }
      //   }
      // }
      // // If the separation dimension does not match a dimension on the RHS,
      // // find the RHS dimension that maps to it to get the corresponding index.
      // let r = genSubs.match(/\[(.*)\]/)
      // if (r) {
      //   let rhsSubs = r[1].split(',').map(x => canonicalName(x))
      //   for (const rhsSub of rhsSubs) {
      //     if (hasMapping(rhsSub, sepDim)) {
