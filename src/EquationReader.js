...
module.exports = class EquationReader extends ModelReader {
  ...
  generateSmoothLevel(input, delay, init, levelNumber) {
    ...
    if (isSeparatedVar(this.var)) {
      ...
      let index = f.modelValue[i]
      // Use the Vensim form of the index in the LHS and in all arguments.
      levelLHS = `${level}[${index}]`
      levelRefId = `${canonicalName(level)}[${s.name}]`
      input = input.replace(genSubscript, index)
      delay = delay.replace(genSubscript, index)
      init = init.replace(genSubscript, index)
    } else {
